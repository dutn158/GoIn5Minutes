...
func PutKey(db storage.DB) http.Handler {
	return http.HandleFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if key == "" {
			http.Error(w, errors.New("missing key name in path"), http.StatusBadRequest)
			return
		}
		...
		if err != nil {
			http.Error(w, errors.New("error reading PUT body"), http.StatusBadRequest)
			return
		}

		if err := db.Set(key, val); err != nil {
			http.Error(w, errors.New("error setting value in DB"), http.StatusInternalServerError)
			return
		}
